...
class Model:
    ...
    def __init__(self, reset, solutionfilename):
        self.checkSolutionFilename(solutionfilename)
        if reset:
            self.writeConfig()
        self.readConfig()

    def checkSolutionFilename(self, filename):
        try:
            with open(filename, 'w') as f:
                pass
            self.solutionFile = filename
        except OSError:
            raise
    ...
    def writeConfig(self):
        ...
        '''
        print(BASE_DIR.child("config.ini"))
        ...
        Config.set('File', 'path', self.solutionFile)
        ...
        Config.set('Geometry', 'l', "[1, 1, 1]")
        Config.add_section('Mesh')
        Config.set('Mesh', '# Set the mesh size: "coarse", "fine" or "normal".')
        Config.set('Mesh', 'size', "normal")
        # mesh type: triangle or mapped, tetra or cube
        ...
        # DIRICHLET, NEUMANN, ROBIN, MIXEDI, MIXEDII
    ...
    def readConfig(self):
        '''
        parameters = {'geom':[2,'1.0','1.0','0.0'],
                      'mesh':'normal',
                      'material':['1000.0', '100.0', '10.0'],
                      'initial': ['u','1.0, 1.0, 1.0'],
                      'source': ['0.0', '0.0', '0.0', '0.1',
                                 'u', '1000.0', '1.0'],
                      'boundary': ['d', 'u', '1.0', '1.0',
                                   'u', '1.0', '1.0', '1.0' '1.0']
                      }
        '''
        Config = configparser.ConfigParser(allow_no_value = True)
